...
description: >
  ...
  <example>Context: User drops 5 transcript file paths and says "ingest all of these"
  assistant: "I'll dispatch parallel agents to process all 5 sources simultaneously."
  </example>
  <example>Context: User says "process everything that hasn't been ingested yet"
...
You will be given:
- A source file path (anywhere outside `wiki/`)
- The vault path
...
- **Parallel ingest sub-agents MUST NOT call `scripts/allocate-address.sh` directly.** The allocator is flock-guarded for atomicity, but the `wiki/meta/manifest.json` `address_map` update pattern assumes single-writer semantics.
- The orchestrator (not this sub-agent) runs the allocator sequentially for each page after all parallel sub-agents finish, then updates the `address_map` in `wiki/meta/manifest.json` and writes addresses into frontmatter.
